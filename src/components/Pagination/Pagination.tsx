...
const getPageButtons = (
  ...
  if (count < 6)
    return Array.from({ length: count }, (_, i) => i + 1).map((item) => (
      <ST.PageButton
        onClick={() => clickHandler({ page: item })}
  ...
  let interval = Array.from(
    { length: endInterval - startInterval + 1 },
    (_, i) => i + startInterval,
  ).map((item) => (
    <ST.PageButton
      onClick={() => clickHandler({ page: item })}
  ...
  if (startInterval > 1) {
    interval =
      startInterval - siblingsCount > 1
        ? [
            <ST.PageButton onClick={() => clickHandler({ page: 1 })}>1</ST.PageButton>,
            <ST.Delimiter>...</ST.Delimiter>,
            ...interval,
          ]
        : [<ST.PageButton onClick={() => clickHandler({ page: 1 })}>1</ST.PageButton>, ...interval];
  }
  if (endInterval < count) {
    interval =
      endInterval + siblingsCount < count
        ? [
            ...interval,
            <ST.Delimiter>...</ST.Delimiter>,
...
const Pagination: FC = ({
  ...
  const pageButtons = useMemo(
    () => getPageButtons(pageCount, +page, setFilterParams, siblingsCount),
