...
import { SideMenuWrapper } from './styled.ts';
...
const Header: FC = () => {
  ...
  return (
    <ST.Wrapper>
      ...
      {openBurger ? <ST.Close onClick={() => setOpenBurger(false)} /> :
        <ST.Burger onClick={() => setOpenBurger(true)} />}
      <AnimatePresence>{openBurger &&
        <>
          <ST.SideMenuBack onClick={() => setOpenBurger(false)}></ST.SideMenuBack>
          <ST.SideMenu key={'side-menu'}
